...
describes what users get. Pick a format with `-o`, or let opensky choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
opensky <command> -o table   # aligned columns for reading
opensky <command> -o jsonl   # one JSON object per line, for piping
opensky <command> -o json    # a single JSON array
opensky <command> -o csv     # spreadsheet friendly
opensky <command> -o tsv     # tab-separated
opensky <command> -o url     # just the URL column
opensky <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
opensky <command> --fields id,title,url
```
...
```bash
opensky <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
opensky <command>            # a table, because this is a terminal
opensky <command> | wc -l    # JSONL, because this is a pipe
```
